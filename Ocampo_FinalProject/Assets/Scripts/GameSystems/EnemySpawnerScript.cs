...
public class EnemySpawnerScript : MonoBehaviour
{
    ...
    [SerializeField] private float _bossHalfSize;
    ...
    private void SpawnEnemy(int number)
    {
        ...
        newEnemy.GetComponent<EnemyStatsScript>().entityName = $"<color=yellow>Enemy #{number}</color>";
        GameManagerScript.instance.AddEnemy(newEnemy.GetComponent<EnemyStatsScript>());
    ...
    private void SpawnBoss()
    {
        ...
        {
            spawnOrigin = new Vector2(Random.Range(-spawnArea.x, spawnArea.x), Random.Range(-spawnArea.y, spawnArea.y));
            colliderWithinArea = Physics2D.OverlapBox(spawnOrigin, new Vector2(_bossHalfSize, _bossHalfSize), 0);
        } while (colliderWithinArea != null);

        // Spawn enemy
        GameObject newEnemy = Instantiate(_bossPrefab, spawnOrigin, Quaternion.identity);
        newEnemy.transform.SetParent(_enemySpawnParent.transform);

        newEnemy.GetComponent<EnemyStatsScript>().entityName = "<color=red>Boss Grenadier</color>";
        GameManagerScript.instance.AddEnemy(newEnemy.GetComponent<EnemyStatsScript>());
    }
}
