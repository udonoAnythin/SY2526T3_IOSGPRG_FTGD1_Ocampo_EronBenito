Shader "Hidden/OceanShader"
{
    Properties
    {
        ...
        _SandColor1 ("SandColor1", Color) = (1, 1, 1, 1)
        _SandColor2 ("SandColor2", Color) = (1, 1, 1, 1)
    }
    SubShader
    {
        // No culling or depth
        ...
        Pass
        {
            ...
            float2 VoronoiNoise(float2 uv, float cellDensity)
            {
                ...
                for (int x = -1; x <= 1; x++)
                {
                    for (int y = -1; y <= 1; y++)
                    {
                        ...
                        float2 randomPoint = neighboringGrid - pointCoordinate + 0.5 + 0.5*sin(randPointCoords * 10);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float value = VoronoiNoise(uv, 3).x;

                // just invert the colors
                col.rgb = lerp(_SandColor1, _SandColor2, VoronoiNoise(uv, 3).y);
                col = floor(col * 16) / 16;
                return col;
            }
            ENDCG
